...
procedure Acerca_De is 
   ...
   Ven_Acerca : Frame_Type := Frame (385, 200, -"Travian Attack Coordinator", 'Z');  
   ...
   Texto1            : Label_Type  := Label (Ven_Acerca, (50, 10), 280, 30, Nombre_Coordinador, Left);  
...
begin
   Situacion_Ventana:=Get_Origin(Ven_Coordinador);
   Set_Origin(Ven_Acerca,
      (Situacion_Ventana.X+(Get_Width(Ven_Coordinador)/2)-(Get_Width(Ven_Acerca)/2),
         Situacion_Ventana.Y+(Get_Height(Ven_Coordinador)/2)-(Get_Height(Ven_Acerca)/2)));
